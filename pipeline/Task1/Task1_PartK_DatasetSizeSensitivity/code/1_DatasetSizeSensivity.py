...
def experiment(threshold, X, y, k):
    skf = StratifiedKFold(n_splits=k, shuffle=True, random_state=0)
    threshold_perf = []
    n_training_examples = []
    fld = 0
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        if threshold == 1:
            ...
            X_train, X_val, y_train, y_val = train_test_split(X_dev, y_dev, test_size=1-threshold, random_state=fld)
        n_training_examples.append(X_train.shape[0])
        best_params = grid_search_kfold("LGBMClassifier", lgbm_clf_param_grid, X_train, y_train, k-1)
        best_params["random_state"] = fld
        auc = test_experiment("LGBMClassifier", X_train, y_train, X_test, y_test, best_params)
        threshold_perf.append(auc)
        fld += 1

    return threshold_perf, n_training_examples
...
if __name__ == "__main__":
    ...
    start = time.time()
    print("running LightGBM on balanced data")

    lightgbm_aucs = []
    n_training_examples = []
    for threshold in np.linspace(0.1, 1, 19):
        print("working on thershold", threshold)
        lightgbm_auc, n_training = experiment(threshold, X, y, k)
        lightgbm_aucs.append(lightgbm_auc)
        n_training_examples.append(n_training)
    pickle.dump(lightgbm_aucs, open("../results/lightgbm_aucs.p", "wb"))
    pickle.dump(n_training_examples, open("../results/n_training_examples.p", "wb"))
    end = time.time()
    duration = end-start
