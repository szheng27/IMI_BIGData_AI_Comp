...
if __name__ == "__main__":
    ...
    query2 = "SELECT * FROM wire_trxns"
    wire_trxns_df = pd.read_sql_query(query2, conn)
    # Close the connection
    conn.close()

    # List of countries
    countries = ['AU', 'CA', 'CN', 'GE', 'IN', 'MX', 'RU', 'SA', 'UK', 'US', 'NAN']
    
    # Initialize new columns in augmented_data_sheet_df for send/receive transactions
    for country in countries:
        augmented_data_sheet_df[f'send_{country}'] = 0
        augmented_data_sheet_df[f'receive_{country}'] = 0

    def update_counters(row):
        sender_country = row['country_sender']
        receiver_country = row['country_receiver']
        
        # Adjust for sender and receiver IDs
        id_sender = row['id_sender']
        id_receiver = row['id_receiver']
        
        # Check if sender or receiver country is in the list, default to 'NAN' otherwise
        sender_country_code = sender_country if sender_country in countries else 'NAN'
        receiver_country_code = receiver_country if receiver_country in countries else 'NAN'
        
        # Update send counter for sender if id_sender matches cust_id in augmented_data_sheet_df
        if id_sender in augmented_data_sheet_df['cust_id'].values:
            augmented_data_sheet_df.loc[augmented_data_sheet_df['cust_id'] == id_sender, f'send_{sender_country_code}'] += 1
        
        # Update receive counter for receiver if id_receiver matches cust_id in augmented_data_sheet_df
        if id_receiver in augmented_data_sheet_df['cust_id'].values:
            augmented_data_sheet_df.loc[augmented_data_sheet_df['cust_id'] == id_receiver, f'receive_{receiver_country_code}'] += 1

    # Apply the function to each row in wire_trxns_df
    wire_trxns_df.apply(update_counters, axis=1)
    ...
    augmented_data_sheet_df.to_csv("../results/Feature_eng_augmented_kyc.csv", index=False)