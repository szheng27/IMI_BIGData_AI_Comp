...
if __name__ == "__main__":
    ...
    catboost_clf_param_grid = {
        "iterations": [50, 100, 200],  # Number of trees to build, similar to 'n_estimators' in XGBoost
        "learning_rate": [0.01, 0.1, 0.2],  # Step size shrinkage used to prevent overfitting
        "depth": [5, 10]  # Depth of the tree. Increasing this value will make the model more complex and more likely to overfit.
        #"l2_leaf_reg": [1, 3, 5, 7],  # Coefficient at the L2 regularization term of the cost function. Similar to 'reg_lambda' in XGBoost.
        #"border_count": [32, 64, 128],  # The number of splits for numerical features. Similar to 'max_bin' in some other libraries.
        #"bootstrap_type": ['Bayesian', 'Bernoulli'],  # Bootstrap type. When set to 'Bayesian', Bayesian bootstrapping is used, allowing for better handling of uncertainty. When set to 'Bernoulli', subsampling is used. Note: Depending on the choice, you may need to adjust other parameters like 'bagging_temperature'.
        # Optional, depending on bootstrap_type
        #"bagging_temperature": [0, 1],  # Controls the degree of data subsampling (Bayesian bootstrapping). Higher values lead to more aggressive bagging.
    }
    start = time.time()
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        best_params = grid_search_kfold("CatBoostClassifier", catboost_clf_param_grid, X_dev, y_dev, k-1)
        best_params["random_state"] = fld
        auc = test_experiment("CatBoostClassifier", X_dev, y_dev, X_test, y_test, best_params)
